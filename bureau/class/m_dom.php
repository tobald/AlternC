...
class m_dom {
    ...
    function domains_type_update($name, $description, $target, $entry, $compatibility, $enable, $only_dns, $need_dns, $advanced, $create_tmpdir, $create_targetdir,$has_https_option=0) {
        global $msg, $db;
        // The name MUST contain only letter, digits and -, it's an identifier after all ...
        if (!preg_match("#^[a-z0-9\-]+$#", $name)) {
            $msg->raise("ERROR", "dom", _("The name MUST contain only letter, digits and -"));
            return false;
        }
    ...
    function clean_https_value($type, $https) {
        ...
        if ($db->Record["has_https_option"]) {
            ...
            if ($https!="http" && $https!="https" && $https!="both") {
                $https=variable_get("http_https_redirection", "https");
            }
            return $https;
        } else return "";
    }
    ...
    function domain_types($sd) {
        ...
        $result = array();
        $php_version = sprintf('php %s', implode(".", array_slice(explode(".", phpversion()), 0, 2)));
        ...
        foreach($this->domains_type_lst() as $dt) {
            ...
            if (strtolower($dt['name']) == 'vhost')
                $entry['display_name'] = $php_version;
